...
import { Code, Palette, Award, Boxes } from "lucide-react";
import { normalizeProject, normalizeDesign, normalizeCertificate } from "../utils/portfolio";
...
const techStacks = [
  ...
  { icon: "adobe-photoshop-icon.png", language: "Adobe Photoshop" },
  { icon: "adobe-premiere-pro-icon.png", language: "Adobe Premiere Pro" },
  { icon: "canva-icon.png", language: "Canva" },
  { icon: "csp.svg", language: "Clip Studio Paint" },
  { icon: "capcut-icon.png", language: "CapCut" },
];
...
export default function FullWidthTabs() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const [projectsResponse, designsResponse, certificatesResponse] = await Promise.all([
      ...
      const designData = (designsResponse.data || []).map(normalizeDesign);
  ...
  useEffect(() => {
    ...
    if (cachedProjects && cachedDesigns && cachedCertificates) {
        setProjects(JSON.parse(cachedProjects).map(normalizeProject));
        setDesigns(JSON.parse(cachedDesigns).map(normalizeDesign));
        setCertificates(JSON.parse(cachedCertificates).map(normalizeCertificate));
    }
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 md:gap-5 gap-4">
                {displayedDesigns.map((design, index) => (
                  <div
                    ...
                    data-aos-duration={index % 3 === 0 ? "1000" : index % 3 === 1 ? "1200" : "1000"}
                  >
                    <Certificate ImgSertif={design.Img} />
                  </div>
                ))}
              </div>
            </div>
