...
import { normalizeCertificate, certificatePayload, createStorageFileName } from '../../utils/portfolio'
...
export default function Certificates() {
  ...
  const uploadImage = async () => {
    if (!file) return
    setUploading(true)
    const fileName = createStorageFileName('cert', file)
    const { error: uploadError } = await supabase.storage.from('certificate-images').upload(fileName, file, { upsert: false })

    if (uploadError) {
      console.error('Failed to upload certificate image:', uploadError.message)
      setUploading(false)
      return
    }

    const { data } = supabase.storage.from('certificate-images').getPublicUrl(fileName)
    const { error: insertError } = await supabase.from('certificates').insert(certificatePayload(data.publicUrl))

    if (insertError) {
      console.error('Failed to save certificate:', insertError.message)
      setUploading(false)
      return
    }
