...
import { Plus, Trash2, Upload, ImageIcon, Palette } from 'lucide-react'
import { normalizeDesign, designPayload, createStorageFileName } from '../../utils/portfolio'
...
export default function Designs() {
  ...
  const fetchDesigns = async () => {
    ...
    setDesigns((data || []).map(normalizeDesign))
    setLoading(false)
  }
  ...
  const uploadImage = async () => {
    if (!file) return
    setUploading(true)
    const fileName = createStorageFileName('design', file)
    const { error: uploadError } = await supabase.storage.from('design-images').upload(fileName, file, { upsert: false })

    if (uploadError) {
      console.error('Failed to upload design image:', uploadError.message)
      setUploading(false)
      return
    }

    const { data } = supabase.storage.from('design-images').getPublicUrl(fileName)
    const { error: insertError } = await supabase.from('designs').insert(designPayload(data.publicUrl))

    if (insertError) {
      console.error('Failed to save design:', insertError.message)
      setUploading(false)
      return
    }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-white">Design & UI UX</h1>
      ...
      <Card>
        <div className="p-5 sm:p-6 space-y-4">
          <h2 className="text-sm font-semibold text-white flex items-center gap-2">
            <Plus className="w-4 h-4 text-indigo-400" /> Upload Design
          </h2>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-4">
          {designs.map(design => (
            <DesignCard key={design.id} design={design} onDelete={deleteDesign} />
          ))}
        </div>
      )}
    </div>
